...
lista concatenar(const lista l1, const lista l2) {
  ...
  while (es_localizador_lista(loc1)){
    info_t info = crear_info(numero_info(info_lista(loc1,l1)), copiar_texto(texto_info(info_lista(loc1,l1))));
    insertar_despues(info,loc2,aux);
    loc2=final_lista(aux);
    loc1=siguiente(loc1,l1);
  }
  loc1=inicio_lista(l2);
  while (es_localizador_lista(loc1)){
    info_t info = crear_info(numero_info(info_lista(loc1,l2)), copiar_texto(texto_info(info_lista(loc1,l2))));
    insertar_despues(info,loc2,aux);
    loc2=final_lista(aux);
...
lista reversa(const lista lst) {
  ...
  while (es_localizador_lista(loc)) {
    ...
    info_t nuevo_nod = crear_info(numero_info(info),copiar_texto(texto_info(info)));
...
lista mezcla(const lista l1, const lista l2) {
  ...
  while(es_localizador_lista(localizador1)&&es_localizador_lista(localizador2)){
    ...
    insertar_despues(info_auxiliar,loc,mez);
    loc=final_lista(mez);
  }
    while(es_localizador_lista(localizador1)){
      info=info_lista(localizador1,l1);
      info_t info2 = crear_info(numero_info(info), copiar_texto(texto_info(info)));
      localizador1=siguiente(localizador1,l1);
      insertar_despues(info2,loc,mez);
      loc=final_lista(mez);
    }
    while(es_localizador_lista(localizador2)){
      info=info_lista(localizador2,l2);
      info_t info3 = crear_info(numero_info(info), copiar_texto(texto_info(info)));
      localizador2=siguiente(localizador2,l2);
      insertar_despues(info3,loc,mez);
      loc=final_lista(mez);
    }
  return mez;
}
...
lista filtrado(const int clave, const comp_t criterio, const lista lst) {
  ...
  while(es_localizador_lista(localizador1)){
    ...
    if (bandera){
      ...
      info_t info1 = crear_info(numero_info(info_auxiliar), copiar_texto(texto_info(info_auxiliar)));
      insertar_despues(info1,final,fil);
    }
...
lista sublista(const nat menor, const nat mayor, const lista lst) {
  ...
  while(es_localizador_lista(localizador1)){
      if (((numero_info(info_lista(localizador1,lst)))<=may) &&  ((numero_info(info_lista(localizador1,lst)))>=men)){
          ...
          info_t info3 = crear_info(numero_info((info2)), copiar_texto(texto_info((info2))));
          localizador ultimo =final_lista(sub);
          insertar_despues(info3,ultimo,sub);
      }
      localizador1=siguiente(localizador1,lst);
  }
  return sub;
