...
void imprimir_textos(const binario b){
  if (!es_vacio_binario(b)){
    ...
    nat nivel_actual = 1;
    nat proximo_nivel = 0;
    pila p = crear_pila(tamanio);
    while (nivel_actual!=0){
      binario raiz = frente(c);
      desencolar(c);
      nivel_actual--;
      apilar(texto_info(raiz_binario(raiz)),p);
      if (!es_vacio_binario(derecho(raiz))){
        encolar(raiz->der,c);
        proximo_nivel++;
      }
      if (!es_vacio_binario(izquierdo(raiz))){
        encolar(raiz->izq,c);
        proximo_nivel++;
      }
      if (nivel_actual==0){
        nivel_actual = proximo_nivel;
        if (proximo_nivel != 0)
          apilar(NULL,p);
        proximo_nivel=0;
